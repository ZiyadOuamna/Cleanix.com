...
-- 
-- IMPORTANT: Les superviseurs SAUTENT la vérification d'email
-- Ils accèdent directement au dashboard après connexion
-- Seul le contrôle d'accès par rôle est appliqué
...
VALUES (
    (SELECT id FROM users WHERE email = 'cleanix.ma.contact@gmail.com' LIMIT 1),
    'Superviseur',
...
SELECT u.id, u.nom, u.email, u.user_type, s.niveau_acces
FROM users u
LEFT JOIN superviseurs s ON u.id = s.user_id
WHERE u.email = 'cleanix.ma.contact@gmail.com';
